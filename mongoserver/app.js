...
//파일 업로드가 있을 때만upload.single() 을 넣는다 .
app.post('/item/insert', upload.single('pictureurl'),
		(req, res, next)=> {
			...
			//Mongo 접속해서 가장 큰 itemid를 찾고 데이터를 삽입
			MongoClient.connect(databaseUrl, function(err, database){
				...
				db.collection('item').find({}, 
						{projection:{_id:0, itemid:1}}).sort({itmeid:-1}).limit(1)
...
app.get('/item/list', (req, res, next) => {
	...
	if(pageno != undefined && count != undefined){
		//자바스크립트는 숫자로 된 문자열 곱하기에 사용하면 숫자로 변환해서 수행
	...
	MongoClient.connect(databaseUrl, function(err, database){
		...
		db.collection('item').find().toArray(function(err, items){
			len = items.length;
			//데이터 가져오는 구문
			db.collection('item').find().sort({'itemid':-1}).skip(start).limit(size).
...
app.get('/img/:fileid', function(req, res){
	...
	//실제 img 디렉토리의 절대 경로를 설정 
	var file = '/Users/a202_09/node/mongoserver/img' + '/' + fileid;
...
app.listen(app.get('port'), () => {
	console.log(app.get('port'), '번 포트로 서버 구동 ');
})
